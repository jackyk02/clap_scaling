#!/usr/bin/env python3
"""
Comprehensive comparison plot showing how action error decreases with different approaches:
...
def generate_comparison_plot():
    """Generate comprehensive comparison plot of all three approaches"""
    ...
    # Define the range of proposal distribution sizes to test
    num_samples_range = [1, 2, 4, 8, 16, 32, 64, 128]
    ...
    ax.plot(num_samples_range, rephrasing_nrmse_means, label='Instruction Rephrasing (Oracle)', 
            color=color_rephrasing, marker=marker_rephrasing, markersize=8, linestyle='-', linewidth=2.5)
    ...
    ax.plot(num_samples_range, variability_nrmse_means, label='Repeated Sampling (Oracle)', 
            color=color_variability, marker=marker_variability, markersize=8, linestyle='-', linewidth=2.5)
    ax.fill_between(num_samples_range, 
    ...
    ax.plot(num_samples_range, gaussian_nrmse_means, label='Gaussian Perturbation (Oracle)', 
            color=color_gaussian, marker=marker_gaussian, markersize=8, linestyle='-', linewidth=2.5)
    ax.fill_between(num_samples_range, 
    ...
    # Title
    ax.set_title('Comparison of Action Generation Approaches\nwith Oracle Selection from Proposal Distributions', 
                 fontsize=18, pad=20)
    
    # Save plot
    output_file = 'action_generation_comparison.png'
    plt.tight_layout()
    ...
    for approach_name, nrmse_means, nrmse_stds in approaches:
        valid_values = [v for v in nrmse_means if v is not None]
        if valid_values:
            best_nrmse = min(valid_values)
            best_idx = nrmse_means.index(best_nrmse)
            best_num = num_samples_range[best_idx]
            best_std = nrmse_stds[best_idx]
            improvement = ((original_avg_nrmse - best_nrmse) / original_avg_nrmse * 100)
            
            print(f"\n{approach_name}:")
            print(f"  Best NRMSE: {best_nrmse:.4f}±{best_std:.4f} (with {best_num} candidates)")
            print(f"  Maximum improvement: {improvement:.1f}%")
    
    # Show detailed breakdown for each approach
    print(f"\n=== Detailed Results by Proposal Distribution Size ===")
    ...
    # Calculate relative improvements between consecutive sizes for each approach
    print(f"\n=== Diminishing Returns Analysis ===")
    for approach_name, nrmse_means, nrmse_stds in approaches:
        print(f"\n{approach_name} - Relative improvement between consecutive sizes:")
        for i in range(1, len(nrmse_means)):
            if nrmse_means[i] is not None and nrmse_means[i-1] is not None:
                prev_nrmse = nrmse_means[i-1]
                curr_nrmse = nrmse_means[i]
                relative_improvement = ((prev_nrmse - curr_nrmse) / prev_nrmse * 100)
                print(f"  {num_samples_range[i-1]:3d} → {num_samples_range[i]:3d}: {relative_improvement:5.1f}% additional improvement")
    ...
    results_df.to_csv('action_generation_comparison_results.csv', index=False)
    print(f"\nNumerical results saved as: action_generation_comparison_results.csv")
    
    # Cross-approach comparison at key sizes
    print(f"\n=== Cross-Approach Comparison at Key Sizes ===")
    key_sizes = [1, 8, 32, 128]
    for size in key_sizes:
        if size in num_samples_range:
            idx = num_samples_range.index(size)
            print(f"\nWith {size} candidates:")
            
            values_at_size = [
                ("Instruction Rephrasing", rephrasing_nrmse_means[idx], rephrasing_nrmse_stds[idx]),
                ("Repeated Sampling", variability_nrmse_means[idx], variability_nrmse_stds[idx]),
                ("Gaussian Perturbation", gaussian_nrmse_means[idx], gaussian_nrmse_stds[idx])
            ]
            
            # Sort by performance (lower NRMSE is better)
            values_at_size.sort(key=lambda x: x[1] if x[1] is not None else float('inf'))
            
            for rank, (approach, nrmse_mean, nrmse_std) in enumerate(values_at_size, 1):
                if nrmse_mean is not None:
                    improvement = ((original_avg_nrmse - nrmse_mean) / original_avg_nrmse * 100)
                    print(f"  {rank}. {approach:20s}: {nrmse_mean:.4f}±{nrmse_std:.4f} NRMSE ({improvement:5.1f}% improvement)")
...
def main():
    ...
    results = generate_comparison_plot()
    
    print(f"\n=== Summary ===")
    print("Generated comprehensive comparison of three action generation approaches:")
