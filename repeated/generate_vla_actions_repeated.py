...
with 16 repeated samples per instruction using temperature=1.0 for sampling variation.
Calculate NRMSE between sampled actions and ground truth actions.
"""
...
def main():
    ...
    results = []
    total_expected = 0
    num_repeats = 5  # Number of repeated samples per instruction
    
    # Count expected datapoints (each instruction repeated 16 times)
    for sample in samples:
        ...
        if original_instruction in rephrase_instructions:
            ...
            total_expected += (1 + len(rephrases)) * num_repeats  # (Original + rephrases) * 16
    
    print(f"Expected total datapoints: {total_expected} ({num_repeats} repeats per instruction)")
    ...
    with tqdm(total=total_expected, desc="Generating actions") as pbar:
        for sample_idx, sample in enumerate(samples):
            ...
            # For each instruction, generate 16 repeated samples
            for instr_idx, instruction in enumerate(all_instructions):
                try:
                    # Create 16 copies of the same instruction for batch processing
                    repeated_instructions = [instruction] * num_repeats
                    
                    # Get 16 actions for the same instruction with temperature=1.0
                    ...
                    # Process each of the 16 generated actions
                    for repeat_idx, (generated_action, output_id) in enumerate(zip(generated_actions, output_ids)):
                        # Calculate NRMSE
                    ...
                    print(f"Error processing instruction '{instruction}' for sample {sample_id}: {e}")
                    # Skip this instruction and update progress bar for all expected repeats
                    pbar.update(num_repeats)
                    continue
    ...
    output_file = f"bridge_openvla_actions_repeated_{timestamp}.json"
    ...
    final_data = {
        'metadata': {
            ...
            'num_repeats_per_instruction': num_repeats,
    ...
    print(f"Original instruction datapoints: {len(original_nrmse)} ({len(original_nrmse)//num_repeats} unique instructions × {num_repeats} repeats)")
    print(f"Rephrased instruction datapoints: {len(rephrase_nrmse)} ({len(rephrase_nrmse)//num_repeats} unique instructions × {num_repeats} repeats)")
